...
Exchange.prototype.bind_pg_watcher = function ()
{
	...
	db_scout.on('notification', function (msg)
	{
		...
		switch ( table ) {
			...
			case 'symbols':
				self.socket.volatile.emit('bidask', {symbol : parts[0], bid : parts[1], ask : parts[2]});
				break;
			case 'matched':
				self.socket.volatile.emit('matched', {symbol : parts[0], price : parts[1], amount : parts[2]});

			// Private:

			case 'balances':
				if ( self.accounts[parts[0]] > 0 ) {
