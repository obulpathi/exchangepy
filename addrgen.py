...
parser = argparse.ArgumentParser(description='Address generation for users')
parser.add_argument("userid");
parser.add_argument("nth");
args = parser.parse_args();

Config = ConfigParser.ConfigParser();
Config.read("addrgen.ini");

secret = Config.get('Addrgen','Secret');

if secret == '':
	raise Exception('Secret phrase missing');
...
def make_passphrase():
	secret = '5K1HkbYffstTZDuV4riUWMbAMkQh57b8798uoy9pXYUDYeUHe7F';             #Very secret phrase here
	return args.userid + secret + args.nth;
...
if __name__ == '__main__':
	print get_addr(gen_eckey(passphrase=make_passphrase(),compressed=True));